...
class Conciliacao(db.Model):
    ...
    data_conciliacao = db.Column(db.DateTime, default=db.func.now())
...
def log_auditoria(acao: str, tabela: Optional[str] = None, registro_id: Optional[int] = None, dados_anteriores: Optional[Dict[str, Any]] = None, dados_novos: Optional[Dict[str, Any]] = None) -> None:
...
def get_estatisticas_conciliacoes():
    try:
        ...
        for i in range(7):
            data = datetime.now(timezone.utc) - timedelta(days=i)
...
def resolver_divergencia(divergencia_id):
    ...
    divergencia.data_resolucao = datetime.now(timezone.utc)
